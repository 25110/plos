...
YAMLProfileWriter::convertNodeProbes(NodeIdToProbes &NodeProbes) {
  ...
  for (auto &[NodeId, Probes] : NodeProbes) {
    ...
    BPI.BlockProbes = std::vector<std::uint64_t>(Probes[0].begin(), Probes[0].end());
    BPI.IndCallProbes = std::vector<std::uint64_t>(Probes[1].begin(), Probes[1].end());
    BPI.CallProbes = std::vector<std::uint64_t>(Probes[2].begin(), Probes[2].end());
